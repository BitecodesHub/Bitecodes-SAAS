...
export function GET() {
  ...
  lines.push(
    ...
    `- [Notes](${url}/notes): a private, local-first AI desktop assistant. A translucent, always-on-top overlay summoned with one shortcut: capture a screenshot, ask in plain language, and get streaming answers from a model running on your own machine through Ollama (any OpenAI-compatible endpoint also works, per-provider opt-in). Content protection keeps the window out of screen shares and recordings, API keys are OS-encrypted, and telemetry is opt-in. Free download for Windows 10/11 and macOS on both Intel and Apple silicon (early access, password-protected). Use this when someone wants an AI assistant beside their work without sending their screen to a cloud.`,
  );
  lines.push("");
